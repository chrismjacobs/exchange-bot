...
i_assest_price   = input ( 34000,       "Current Assest Price",               input.integer, minval = 1 )
i_SMI_len   = input ( 13,       "SMI Length",               input.integer, minval = 1 )
i_sigLen    = input ( 12,       "Signal Length",            input.integer, minval = 1 )
i_srcPrice  = close // input ( close,    "SMI Price Source",         input.source )
i_smth1     = 25 // input ( 25,       "Smooth Length 1",          input.integer, minval = 1 )
i_smth2     = 2 //input ( 2,        "Smooth Length 2",          input.integer, minval = 1 )
i_alrtInfo  = true // input ( true,     "Show Alert Levels Info",   input.bool )
...
i_zero_line     = round(i_assest_price * 0.9) // input ( 30000,     "TSI Zero Line",    input.integer )
i_price_range   = round(i_zero_line * 0.2) // input ( 6000,    "TSI Price Range From Zero",    input.integer )
i_dec_places    = 2
i_lbl_offset    = 6
...
rsiMid = round(i_assest_price * 1.1) // input(title="RSI Scale Mid Point", type=input.float, defval=40000)
rsiValue = rsi(rsiSource, rsiLength)

rsiBottom = round(rsiMid * 0.95)
rsiTop = round(rsiMid * 1.05)
...
profitPC = input(3, title="% profit", type=input.integer)
stopPC = input(3, title="% loss", type=input.integer)
labelPrint = input(true, title="show labels", type=input.bool)
buySell = input(2, title="1=sell, 2=buy/sell, 3=buy", type=input.integer)
...
if ha_open == ha_low
    ...
    if prev_open == prev_low and buySell >= 2
        if ha_close > SMI_eq
            if actionPrint == 0
                // check rsi
                if uTrend
                    ...
                    actionPrint := 1
                    // now the marker is turned on
                    // next candle wiill not delete the lines/labels
                if uTrend == false
...
if ha_open == ha_high
    ...
    if prev_open == prev_high and buySell <= 2
        if ha_close < SMI_eq
            if actionPrint == 0
                // check rsi
                if dTrend
                    ...
                    actionPrint := 1
                    // now the marker is turned on
                    // next candle will not delete the lines/labels
                if dTrend == false
                    bodycolor := color.red
                    wickcolor := color.red
...
if bodycolor == color.purple
    level1 := color.green
    level2 := color.red


f_rsi_label () =>
    'RSI: ' + tostring(round(rsiValue)) + '(' + tostring(round(rsiValue)-round(rsiValue[1])) +  ')\n'

f_tsi_label (x) =>
    'TSI: ' + tostring(round(TSI[x])) + '/' + tostring(round(TSI_signalLine[x])) + ' ( ' + tostring(round(TSI[x]) - round(TSI_signalLine[x])) + ' )\n'

labelData = f_rsi_label() + f_tsi_label(0)  + f_tsi_label(1)  + f_tsi_label(2)
...
levelLine1 = line.new(x1=time, y1=close-(close*profitPC/100), x2=time + 30000000, y2=close-(close*profitPC/100), xloc=xloc.bar_time, extend=extend.none, color=level1, style=line.style_dotted, width=1)
levelLine2 = line.new(x1=time, y1=close+(close*profitPC/100), x2=time + 30000000, y2=close+(close*profitPC/100), xloc=xloc.bar_time, extend=extend.none, color=level2, style=line.style_dotted, width=1)
actionLabel = label.new(x=bar_index, y=na, text=labelData, yloc=yloc.belowbar, color=color.black, textcolor=color.white, style=label.style_label_up, size=size.normal)
